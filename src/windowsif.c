...
/*=============================================================
  ...
  Contains all WINDOWS specific interface routines
...
openfile(char *flnm, char modechar, char type)
  ...
  *
  * modechar may be one of:
  * 'r'     open for input.  The file must exist.
  * 'w'     create new file for output.
  * 'a'     append to existing file, or create if file does not exist.
  *         Writes will cause data to be appended to the end of
  *         the file.
...
{
  FILE       *fnm;
  char       openmode[128];       /* Build the actual mode here */
  char       *mode = openmode;
  
  /* work out the actual open mode */
  openmode[0] = '\000'; /* Clear */
  switch(modechar) {
    case 'r':
    case 'w':
    case 'a':
    ...
    case 'd':
    case 'i':
      /* Determine mode based on accessibility of file */
      if (access(flnm, 00) == 0)
        mode = "r+b";
      else
        mode = "w+b";
  ...
  if ((fnm = fopen(flnm, mode)) == NULL) {
    ...
    if (modechar == 'd' || modechar == 'i') {
      /* go to end of file */fseek(fnm, 0L, 2);
    }
...
/** The following gets the wall clock time NEEDS WORK */
double
get_cputime()
{
...
void ios_get_parameter() {
  ...
  switch(intval(x)) {
    ...
    case 2:
      ...
      apush(makestring(getenv("USERPROFILE")));
      break;
