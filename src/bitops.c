...
void iacc_xor(void) {
  ...
  unialint zwd;
  unialint acc = 0;
  unialint mask = NIALONEBIT << (boolsPW-1);
  int v;
  ...
  if (bc <= 1) {
    ...
    v = valence(x);
    shp = shpptr(x, v);
    res = new_create_array(booltype, v, 0, shp);
    
    /* Determine parameters of the operation */
    wds = bc / boolsPW;         /* whole words */
    exc = bc % boolsPW;         /* excess bits */
